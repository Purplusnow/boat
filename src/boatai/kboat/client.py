"""공공데이터포털(data.go.kr) 경정 오픈API 클라이언트.

경정 API 는 국민체육진흥공단 경주사업총괄본부가 **GW(게이트웨이) 방식**으로
공개한다. 같은 포털이지만 한국마사회 API 와 규칙이 다르고, 그 차이가 그대로
``NO_OPENAPI_SERVICE_ERROR`` 의 원인이 된다:

  * 호스트에 **서비스 세그먼트가 포함**된다.
    ``/B551014/SRVC_OD_API_VWEB_MBR_RACE_INFO/TODZ_API_VWEB_MBR_RACE_I``
    마사회처럼 ``/B551015/API26_2/entrySheet_2`` 를 기대하고 짧은 경로로
    부르면 게이트웨이가 서비스를 못 찾아 코드 12 를 돌려준다.
  * 응답 형식 파라미터가 ``_type`` 이 아니라 **``resultType``** 이고 **필수**다.
    빠지면 기본값 XML 로 응답해 JSON 파서가 조용히 빈 결과를 만든다.
  * ``pageNo`` · ``numOfRows`` 도 대부분 **필수**다. 마사회는 생략해도 됐다.
...
CODE_HINT = {
    ...
    "30": "등록되지 않은 인증키다. .env 의 KBOAT_SERVICE_KEY 를 확인하라.",
...
class KboatApiError(RuntimeError):
...
def read_service_key() -> str:
    ...
    않는다.
    """
    for name in ("KBOAT_SERVICE_KEY", "KRA_SERVICE_KEY"):
    ...
    for base in (Path.cwd(), Path(__file__).resolve().parents[3]):
        ...
        for line in env.read_text(encoding="utf-8").splitlines():
            ...
            if name.strip() in ("KBOAT_SERVICE_KEY", "KRA_SERVICE_KEY"):
...
def normalize_service_key(raw: str) -> str:
    ...
    if not key:
        raise ValueError(
            "서비스키가 비어 있습니다. .env 의 KBOAT_SERVICE_KEY 를 확인하세요."
        )
...
class KboatClient:
    ...
    @classmethod
    def from_env(cls, **kw) -> "KboatClient":
    ...
    def __post_init__(self):
        ...
        self.session.headers.update(
            {"User-Agent": "boatai/1.0 (+data.go.kr open api client)"}
        )
    ...
    def raw(self, path: str, params: Dict[str, Any]) -> dict:
        ...
        for attempt in range(self.max_retries):
            ...
            if err:
                exc = KboatApiError(err[0], err[1], url)
                if exc.fatal:
                    raise exc
                last_exc = exc
                time.sleep(1.5 * (attempt + 1))
            ...
            if r.status_code == 429:
                raise KboatApiError("22", "호출 한도 초과 (일일 트래픽 제한)", url)
            if r.status_code >= 500:
                last_exc = KboatApiError(str(r.status_code), "server error", url)
                time.sleep(1.5 * (attempt + 1))
                continue
            if r.status_code >= 400:
                # raise_for_status 는 URL 을 그대로 담아 키를 노출한다.
                raise KboatApiError(str(r.status_code), f"HTTP {r.status_code}", url)

            try:
                data = r.json()
            except ValueError:
                last_exc = KboatApiError("PARSE", f"JSON 파싱 실패: {text[:200]}", url)
            ...
            if code not in OK_CODES:
                exc = KboatApiError(code, str(header.get("resultMsg", "")), url)
                if exc.fatal:
                    raise exc
                last_exc = exc
                time.sleep(1.5 * (attempt + 1))
        ...
        # 네트워크 예외도 KboatApiError 로 감싸 내보낸다. 호출자가 requests 예외까지
        # 따로 잡아야 한다면, 한 소스의 일시적 실패가 배치 전체를 죽인다.
        if isinstance(last_exc, KboatApiError):
            raise last_exc
        raise KboatApiError("NETWORK", f"요청 실패: {type(last_exc).__name__}", url) from last_exc
