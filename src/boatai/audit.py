...
def check(conn, days: int) -> List[Dict]:
    ...
    # ── 출주표 ───────────────────────────────────────────────────
    # **아직 안 열린 경주는 세지 않는다.** 일정은 회차가 확정되는 즉시 들어오지만
    # 출주표는 경주 하루이틀 전에야 나온다. 그 사이를 결손으로 세면 매주 34경주가
    # 거짓 경보로 뜬다. 여기서 잡을 것은 '이미 시행됐는데 출주표가 없는' 경주다.
    add("출주표 결손", df[(df["n_entry"] == 0) & (df["n_ord"] > 0)],
        "착순으로만 알게 된 경주다 — 정번을 붙일 수 없어 피처도 평가도 못 만든다")
    # 당일인데 아직 출주표가 없으면 예상을 만들 창이 닫히는 중이다. 앞날 것은
    # 정상이므로 오늘 이전만 본다.
    add("출주표 미게시", df[(df["n_entry"] == 0) & (df["n_ord"] == 0) &
                        (df["race_ymd"] <= today_kst().strftime("%Y%m%d"))],
        "경주일인데 출주표가 아직 없다 — 발주 전 예상을 만들 수 없다")
