...
평가**다. 경정 예측은 그럴듯한 숫자를 만들기는 쉽고 실제로 쓸모 있기는 어렵다.
그래서 둘을 강제한다.

1. **워크포워드 검증** — 시간 순서대로 과거로 학습해 미래를 맞힌다. 무작위
   분할은 같은 경주의 다른 배가 학습셋에 들어가 성능을 부풀린다.

2. **베이스라인 동시 측정** — 경마 버전은 1인기(최저 단승배당)를 베이스라인으로
   썼지만, **경정 공개 API 에는 발주 전 배당률이 없다.** 단승 배당은 적중한
   조합의 것만, 그것도 경주 후에 공개된다. 그래서 시장 대신 두 가지를 쓴다.

   * **1코스 고정** — 경정에서 1코스는 인쪽 최단거리로 선회해 승률이 압도적으로
     높다. 아무 생각 없이 1번만 찍어도 상당히 맞는다. 이걸 못 이기면 모델이
     배운 것이 없다는 뜻이다.
   * **최근 폼 최고** — 최근 6회차 연대율이 가장 높은 배. 사람이 출주표만 보고
     고르는 방식에 가깝다.
...
def normalize_within_race(df: pd.DataFrame, prob_col: str, out_col: str) -> pd.DataFrame:
    ...
    이진 분류기는 배 단위로 독립 예측하므로 합이 1이 아니다. 경주는 정확히 한
    척만 이기므로 정규화해야 확률로 읽을 수 있다.
    """
    df = df.copy()
    p = pd.to_numeric(df[prob_col], errors="coerce").clip(1e-6, 1 - 1e-6)
    total = p.groupby(df["race_key"]).transform("sum")
    n = df.groupby("race_key")["lane"].transform("count")
...
class Eval:
    n_races: int
    n_runners: int
    top1_win: float           # 1순위 추천정의 단승 적중률
    top1_top2: float          # 1순위가 2착 이내 (경정 연승 = 2착까지)
    top2_quinella: float      # 상위 2정이 실제 1·2착 (순서 무관) = 복승
    top3_trio: float          # 상위 3정이 실제 1·2·3착 (순서 무관) = 삼복승
    top3_has_winner: float
    logloss: float
    auc: float
    roi_win: float            # 1순위 단승 100원 베팅 회수율
    # 베이스라인
    lane1_win: float          # 1코스 고정
    lane1_roi: float
    form_win: float           # 최근 6회차 연대율 최고
...
def evaluate(pred: pd.DataFrame) -> Eval:
    """경주 단위 지표. pred 는 p_win_norm, ord, lane, win_payout 을 포함."""
    df = pred.copy()
    ...
    top3_has_winner = float(top3.groupby("race_key")["ord"].apply(lambda s: (s == 1).any()).mean())
    ...
    # ROI — 배당이 들어온 경주만. 배당 자료가 없는 경주를 '0원 회수'로 세면
    # 회수율이 자료 결손만큼 낮게 나온다 (경마 버전에서 실제로 겪은 사고다).
    ...
    # ── 베이스라인 ───────────────────────────────────────────────
    lane1 = df[df["lane"] == 1]
    lane1_win = float((lane1["ord"] == 1).mean()) if len(lane1) else float("nan")
    lane1_roi = _roi(lane1)

    form_col = "tms6_high_rate" if "tms6_high_rate" in df else None
    if form_col:
        df["form_rank"] = df.groupby("race_key")[form_col].rank(ascending=False, method="first")
        f1 = df[df["form_rank"] == 1]
        form_win = float((f1["ord"] == 1).mean()) if len(f1) else float("nan")
    else:
        form_win = float("nan")
    ...
    return Eval(
        ...
        lane1_win=lane1_win, lane1_roi=lane1_roi, form_win=form_win,
...
def predict_frame(models: Dict, df: pd.DataFrame, cols: List[str]) -> pd.DataFrame:
    ...
    n = out.groupby("race_key")["lane"].transform("count")
    ...
    # (실제로 화면에 '2착이내 84.0% · 3착이내 81.4%' 가 나갔다). 독립 모델
    # 셋의 산출물을 확률로 읽으려면 이 관계는 지켜져야 한다.
...
def walk_forward(df: pd.DataFrame, cols: List[str], n_folds: int = 5,
                 min_train_races: int = 2000, seed: int = 42) -> Tuple[List[Eval], pd.DataFrame]:
    """시간순 확장 학습 검증."""
    ...
    for i in range(n_folds):
        ...
        log.info("fold %d | 학습 %d경주 | 검증 %d경주 → 단승 %.1f%% "
                 ...
                 ev.top1_win * 100, ev.lane1_win * 100, ev.form_win * 100)
...
def load(path: Path = MODEL_PATH) -> Dict:
    ...
    if not path.exists():
        raise FileNotFoundError(f"학습된 모델이 없습니다: {path}  (python -m boatai.model train)")
...
def report(s: Dict) -> str:
    if not s:
        return "검증 결과가 없습니다."
    lines = [
        f"검증 경주 {s['n_races']:,}회 / 출주 {s['n_runners']:,}정  (fold {s['folds']})",
        "",
        f"{'지표':<26}{'AI 모델':>10}{'1코스 고정':>12}{'최근폼 최고':>12}",
        "-" * 62,
        f"{'단승 적중률':<26}{_fmt(s.get('top1_win')):>10}"
        f"{_fmt(s.get('lane1_win')):>12}{_fmt(s.get('form_win')):>12}",
        f"{'단승 회수율(ROI)':<26}{_fmt(s.get('roi_win')):>10}"
        f"{_fmt(s.get('lane1_roi')):>12}{'':>12}",
        "-" * 62,
        f"{'연승 (1순위 2착 이내)':<26}{_fmt(s.get('top1_top2')):>10}",
        f"{'복승 (상위 2정)':<26}{_fmt(s.get('top2_quinella')):>10}",
        f"{'삼복승 (상위 3정)':<26}{_fmt(s.get('top3_trio')):>10}",
        f"{'상위 3정 안에 1착':<26}{_fmt(s.get('top3_has_winner')):>10}",
        "-" * 62,
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="경정 승률 모델 학습/검증")
    ...
    ap.add_argument("--since-year", type=int, default=None,
                    help="이 해부터만 학습 (오래된 규정·정원 변화를 배제)")
    ...
    if df.empty:
        print("학습 데이터가 없습니다. 먼저 수집하세요:\n"
              "  python -m boatai.kboat.collect backfill", file=sys.stderr)
        return 1
    if args.since_year:
